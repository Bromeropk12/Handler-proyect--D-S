...
class Sample(Base):
    ...
    linea_negocio = Column(String(50), nullable=False, default="cosméticos")
    clase_peligro_id = Column(
        Integer, ForeignKey("clases_peligro.id"), nullable=True, index=True
    )

    # Cantidad y dimensiones
    cantidad_gramos = Column(Numeric(10, 2), nullable=False, default=0)
    dimension = Column(String(20), nullable=False, default="1x1")
    ...
    sample_parent_id = Column(
        Integer, ForeignKey("muestras.id"), nullable=True, index=True
    )
