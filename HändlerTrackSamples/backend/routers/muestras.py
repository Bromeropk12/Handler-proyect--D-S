...
async def list_muestras(
    ...
    linea_negocio: Optional[str] = Query(
        None, description="Filtrar por línea de negocio"
    ),
    clase_peligro_id: Optional[int] = Query(
        None, description="Filtrar por clase de peligro"
    ),
...
async def get_muestra(
    ...
    muestra = (
        db.query(Sample)
        .options(joinedload(Sample.proveedor), joinedload(Sample.clase_peligro))
        .filter(Sample.id == muestra_id)
        .first()
    )
...
async def create_muestra(
    ...
    if muestra.proveedor_id:
        proveedor = (
            db.query(Proveedor)
            .filter(Proveedor.id == muestra.proveedor_id, Proveedor.activa == True)
            .first()
        )
        if not proveedor:
            raise HTTPException(
    ...
    if muestra.clase_peligro_id:
        clase = (
            ...
            .filter(
                ClasePeligro.id == muestra.clase_peligro_id, ClasePeligro.activa == True
            )
            .first()
        )
        if not clase:
    ...
    if muestra.fecha_manufactura and muestra.fecha_vencimiento:
        ...
        fecha_manuf = muestra.fecha_manufactura
        fecha_venc = muestra.fecha_vencimiento

        # Si son strings, convertir
        if isinstance(fecha_manuf, str):
            fecha_manuf = datetime.fromisoformat(fecha_manuf.replace("Z", "+00:00"))
        if isinstance(fecha_venc, str):
            fecha_venc = datetime.fromisoformat(fecha_venc.replace("Z", "+00:00"))

        if isinstance(fecha_venc, datetime) and isinstance(fecha_manuf, datetime):
            if fecha_venc < fecha_manuf:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="La fecha de vencimiento no puede ser anterior a la fecha de manufactura",
                )

    # Crear muestra (el modelo ahora acepta strings directamente)
    db_muestra = Sample(**muestra.model_dump(), created_by=current_user.id)
    db.add(db_muestra)
    db.commit()
...
async def update_muestra(
    ...
    if "proveedor_id" in update_data and update_data["proveedor_id"]:
        proveedor = (
            db.query(Proveedor)
            .filter(
                Proveedor.id == update_data["proveedor_id"], Proveedor.activa == True
            )
            .first()
        )
        if not proveedor:
    ...
    if "dimension" in update_data and update_data["dimension"]:
        ...
        if update_data["dimension"] not in valid_dimensiones:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Dimensión inválida"
            )
    ...
    if "estado" in update_data and update_data["estado"]:
        ...
        if update_data["estado"] not in valid_estados:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Estado inválido"
            )

    # Actualizar campos directamente (el modelo ahora acepta strings)
...
async def delete_muestra(
    ...
    # Soft delete - usar string directamente
...
async def get_muestras_stats(
    ...
    total_gramos = (
        ...
        .with_entities(Sample.cantidad_gramos)
        .all()
    )
