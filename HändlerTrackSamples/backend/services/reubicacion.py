...
class ReubicacionService:
    ...
    def sugerir_intercambio(
        ...
        intercambiable = (
            (int(dim_a[0]) <= hilera_b.ancho_max and int(dim_a[1]) <= hilera_b.fondo_max) and
    ...
    def buscar_muestras_inertes(
        ...
        clases_bajas = db.query(ClasePeligro).filter(
            ClasePeligro.codigo.in_(["GHS01", "GHS09"])  # Explosivo y Peligros para el medio ambiente
        ).all()
    ...
    def get_propuestas_reubicacion(
        ...
        if not muestra.hilera_id:
            return {
                "success": False,
                "error": "La muestra no está asignada a una hilera"
            }
        
        # Esta implementación retornará propuestas básicas
        # La implementación completa incluirá análisis de compatibilidad
        hilera_actual = db.query(Hilera).filter(Hilera.id == muestra.hilera_id).first()
