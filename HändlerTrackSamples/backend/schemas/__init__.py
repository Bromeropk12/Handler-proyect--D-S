...
# ============ Esquemas para Reset de Contraseña ============


class PasswordResetRequest(BaseModel):
    """Schema para resetear contraseña usando palabra clave"""

    username: str = Field(
        ..., min_length=1, max_length=50, description="Nombre de usuario a resetear"
    )
    palabra_clave: str = Field(..., description="Palabra clave de administrador")
    new_password: str = Field(..., description="Nueva contraseña")
    confirm_password: str = Field(
        ..., description="Confirmación de la nueva contraseña"
    )
...
class PasswordResetResponse(BaseModel):
    """Response para el reset de contraseña"""
...
class ProveedorBase(BaseModel):
    ...
    lineas_negocio: Optional[List[str]] = Field(
        default_factory=list, description="Líneas de negocio que atiende el proveedor"
    )
...
class SampleBase(BaseModel):
    ...
    linea_negocio: str = "cosméticos"
    clase_peligro_id: Optional[int] = None
    cantidad_gramos: Optional[Decimal] = Field(default=0, ge=0)
    dimension: str = "1x1"
    fecha_manufactura: Optional[Union[datetime, str]] = None
    fecha_vencimiento: Optional[Union[datetime, str]] = None
...
class RegistroEntradaRequest(BaseModel):
    ...
    hilera_destino_id: Optional[int] = Field(
        None, description="ID de hilera destino (opcional)"
    )
    observaciones: Optional[str] = None
